...
class DataLoader:
    ...
    def __init__(
        ...
        *,
        ...
        if use_iid:
            self.train_datasets = self._split_dataset_into_subsets_iid(mnist_train, train_indices, self.n_clients)
        else:
            self.train_datasets = self._split_dataset_into_subsets_non_iid(mnist_train, train_indices, self.n_clients)
        ...
        self._train_loaders = self._create_data_loaders(self.train_datasets, self.batch_size, device)
    ...
    def _split_dataset_into_subsets_non_iid(
        ...
        for _, indices in label_to_indices.items():
            ...
            for client_id in range(n_clients):
                client_specific_indices = indices[allocated_indices : allocated_indices + min_samples_per_label]
        ...
        for _, indices in label_to_indices.items():
            remaining_indices = indices[allocated_indices:]  # Get remaining indices after min allocation
            ...
            proportions_end_index = [int(proportion * n_remaining_indices) for proportion in proportions_remaining]
